...
/***/ function(module, exports, __webpack_require__) {
	...
	var _feed = __webpack_require__(2);

	var _feed2 = _interopRequireDefault(_feed);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var authObj = new _auth2.default();
	var feedObj = new _feed2.default();
	...
	var Auth = function () {
	  ...
	  _createClass(Auth, [{
	    ...
	    value: function checkLoginState() {
	      ...
	      var cb = this.statusChangeCallback.bind(this);
	...
	var Feed = function () {
		...
		_createClass(Feed, [{
			...
			value: function reset() {
				...
				var rootNode = document.getElementById('feeds');
				rootNode.innerHTML = "";
					...
					if (response && !response.error) {
						...
						if (response.paging) {
							if (response.paging.next) {
								_this.getMore(response.paging.next).then(function (d) {
									_this.addToList(d);
								});
							}
						} else {
							console.log("REACH END." + _this.postCount);
							_this.finish();
						}
					}
				});
			...
			value: function getMore(url) {
				...
				return new Promise(function (resolve, reject) {
					_superagent2.default.get(url).end(function (err, res) {
						...
						if (d.paging) {
							if (d.paging.next && _this2.postCount < MaxPosts) {
								_this2.getMore(d.paging.next).then(function (d) {
									console.log("GetMore:");
									console.log(d);
									_this2.addToList(d);
								});
							...
							console.log("REACH END." + _this2.postCount);
							_this2.finish();
						}
						resolve(d.data);
					});
			...
			key: 'shouldAdd',
			value: function shouldAdd(entry) {
				console.log(entry);
			...
			value: function addToList(data) {
				...
				data.forEach(function (entry) {
					console.log("- postCount " + _this3.postCount);
					//show progress
					_this3.progressIndexNode.innerHTML = "";
					_this3.progressIndexNode.appendChild(document.createTextNode(_this3.postCount));

					if (_this3.postCount < MaxPosts) {

						_this3.postCount++;
						entry.index = _this3.postCount;
						console.log(entry);

						//get likes
						_post2.default.getLikeCount(entry.id).then(function (d) {
							entry.totalCount = d;
							//add to list
							_this3.myPosts.push(entry);
						});
					}
				});
			...
			value: function addToDom(entry) {
				...
				var left_block_node = document.createElement("div");
				left_block_node.classList.add("feed-left");

				var index_node = document.createElement("div");
				index_node.appendChild(document.createTextNode(entry.index));

				var right_block_node = document.createElement("div");
			...
			value: function finish() {
				...
				//add to dom
				this.myPosts.sort(function (a, b) {
					return a.index - b.index;
				});
				this.myPosts.map(function (a) {
					_this4.addToDom(a);
				});
			...
			value: function download() {
				...
				//JSON
				var json = JSON.stringify(this.myPosts, null, 4);
				var blob = new Blob([json], { type: 'application/json' });
				var url = URL.createObjectURL(blob);
				var j = document.getElementById('downloadJSON');
				...
				this.myPosts.forEach(function (d) {
					var content = d.message || d.story;
					if (content) {
				...
				var c = document.getElementById('downloadCSV');
...
/***/ function(module, exports) {
	...
	var Post = function () {
		...
		_createClass(Post, null, [{
			key: 'getLikeCount',
			value: function getLikeCount(id) {
